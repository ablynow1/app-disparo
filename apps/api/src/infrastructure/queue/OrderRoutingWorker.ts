...
export const orderRoutingWorker = new Worker<ProcessOrderJobData>(
  ...
  async (job: Job<ProcessOrderJobData>) => {
    ...
    logger.info(`[OrderWorker] Orquestrando Regra para o Pedido DB interno ID: ${orderId}`);
    ...
    const connectedJids = activeRule.instances.map((i: any) => i.remoteJid!).filter(Boolean);
    ...
    logger.info(`[OrderWorker] ✅ Pedido Orquestrado com sucesso. A regra disparará magicamente pela Instância: ${nextWhatsappInstance}`);
  },
  {
