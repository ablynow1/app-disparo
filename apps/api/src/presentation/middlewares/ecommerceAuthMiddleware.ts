...
export async function ecommerceAuthMiddleware(request: FastifyRequest, reply: FastifyReply) {
  ...
  if (path.includes('shopify')) {
    ...
    const token = request.headers['x-yampi-token'] || (request.query as any).token;
    if (!token) {
    ...
    const token = (request.query as any).access_token || (request.query as any).token;
    if (!token) {
