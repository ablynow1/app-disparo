...
export function normalizeAppmaxPayload(payload: any): StandardOrder {
  ...
  if (event === 'OrderPaid' || data.status === 'aprovado') status = 'PAID';
  if (event === 'OrderCanceled' || data.status === 'recusado' || data.status === 'cancelado') status = 'CANCELED';
  if (event === 'OrderCreated' || event === 'OrderBilletPrinted' || data.status === 'pendente') status = 'PENDING';
