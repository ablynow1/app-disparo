...
import { Brain, Trash2, PlusCircle, Save, X, Network, FileText, Zap, MessageSquareQuote } from 'lucide-react';
...
export function AgentsClient({ initialAgents }: { initialAgents: any[] }) {
  ...
  const [playgroundAgent, setPlaygroundAgent] = useState<any | null>(null);
  ...
  return (
    <>
        ...
        {agents.map(agent => (
          <div key={agent.id} className="bg-zinc-950/50 border border-zinc-800/60 rounded-3xl p-6 relative group hover:border-indigo-500/30 transition-colors">
            ...
            <div className="mt-6 flex gap-2">
              <button onClick={() => setPlaygroundAgent(agent)} className="flex-1 bg-indigo-600/10 hover:bg-indigo-600/20 text-indigo-400 border border-indigo-500/20 text-sm py-2 rounded-xl transition-colors font-medium flex items-center justify-center gap-2">
                <MessageSquareQuote size={16} /> Simulador
              </button>
              <button onClick={() => openForm(agent)} className="w-10 flex items-center justify-center bg-zinc-900 border border-zinc-800 hover:bg-zinc-800 text-zinc-300 rounded-xl transition-colors font-medium">
                <Brain size={16} />
              </button>
      ...
      <AnimatePresence>
        {playgroundAgent && (
          <AiPlaygroundModal agent={playgroundAgent} onClose={() => setPlaygroundAgent(null)} />
        )}
      </AnimatePresence>
    </>
  );
