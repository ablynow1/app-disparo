...
export default function DashboardLayout({
  ...
  const navItems = [
    ...
    { name: 'Mensagens', href: '/dashboard/chat', icon: MessageSquare, section: 'Atendimento' },
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex overflow-hidden selection:bg-indigo-500/30 relative">

      {/* 
        =========================================================
        GLOBAL NEON GLOW ORBS (BACKGROUND GLASSMORPHISM ENGINE)
        =========================================================
      */}
      ...
      <div className="absolute top-[20%] right-[-5%] w-[30%] h-[50%] rounded-full bg-purple-600/10 blur-[120px] pointer-events-none mix-blend-screen" />
      <div className="absolute bottom-[-10%] left-[20%] w-[50%] h-[40%] rounded-full bg-blue-600/10 blur-[120px] pointer-events-none mix-blend-screen" />
      ...
      {/* 
        ...
        ---------------------------------------------------------
      */}
      <aside className="hidden md:flex flex-col w-72 border-r border-white/5 bg-background/40 backdrop-blur-3xl relative z-40 shadow-[4px_0_24px_-10px_rgba(0,0,0,0.5)]">
        ...
        <nav className="flex-1 px-4 py-8 space-y-6 overflow-y-auto custom-scrollbar">
          {['Principal', 'Atendimento', 'Motor de Vendas', 'Inteligência RAG'].map((section) => (
              ...
              {navItems.filter(i => i.section === section).map((item) => {
                ...
                return (
                  <Link
                    key={item.href}
                    href={item.href}
                    className={`flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-2xl transition-all duration-300 relative group ${isActive
                      ? 'text-white bg-white/[0.08] shadow-sm ring-1 ring-white/10'
        ...
        <div className="p-6 border-t border-white/5 mt-auto bg-background/20 backdrop-blur-md">
          <form action={async () => {
            const { logOut } = await import('@/actions/auth');
            await logOut();
          }}>
            <button type="submit" className="flex items-center gap-3 w-full px-4 py-3 text-sm font-medium rounded-2xl text-red-400/80 hover:text-red-400 hover:bg-red-500/10 transition-colors group">
      ...
      */}
      <main className="flex-1 flex flex-col relative w-full h-[100dvh]">
        ...
        <div className="absolute top-0 inset-x-0 h-96 bg-gradient-to-b from-indigo-500/5 to-transparent pointer-events-none mix-blend-screen" />

        {/* Header Superior Top Bar (ULTRA GLASS) */}
        <header className="h-20 border-b border-white/5 bg-background/40 backdrop-blur-2xl flex items-center justify-between px-6 lg:px-10 sticky top-0 z-30 w-full shadow-[0_4px_24px_-10px_rgba(0,0,0,0.5)]">
      ...
      {/* 
        ...
        ---------------------------------------------------------
      */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 h-[84px] pb-safe border-t border-white/10 bg-background/60 backdrop-blur-3xl z-50 px-2 shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.5)]">
