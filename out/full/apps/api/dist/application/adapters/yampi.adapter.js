...
exports.normalizeYampiPayload = normalizeYampiPayload;
function normalizeYampiPayload(payload) {
    let status = 'PENDING';
    ...
    if (yampiStatus === 'paid')
        status = 'PAID';
    if (yampiStatus === 'canceled' || yampiStatus === 'refunded')
        status = 'CANCELED';
    if (yampiStatus === 'pending' || yampiStatus === 'waiting_payment')
        status = 'PENDING';
